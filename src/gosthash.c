...
#define _aligned_free(size) free(size)
#endif
...
void UADD(v256 *r, const v256 *a)
{
    ...
    {//   register uint64_t t = (uint64_t)r->s[i] + a->s[i] + cy;
        cy = _addcarry_u64(cy, r->d[i], a->d[i], &r->d[i]);
    }
}

static inline
void UADD1(v256 *a, v256 *b)
{
    uint64_t ac;
    __asm volatile (
    "   movq   (%1), %0   \n" // ac = a[i]
...
#include <x86intrin.h>
//static inline
void UADD1(v256 *r, const v256 *a)
{
    int i;
    unsigned char cy = 0;
    for (i=0; i<8; i++)
    {//   register uint64_t t = (uint64_t)r->s[i] + a->s[i] + cy;
        cy = _addcarry_u32(cy, r->s[i], a->s[i], &r->s[i]);
    }
}
...
#endif
static inline v2di SLL128U(v2di a, const int bits) {

    return (v2di)__builtin_ia32_pslldqi128((v2di)a, bits);
}
static inline
void A(v256 *x)
{
...
const v2di Z={0};
    ...
    //v1[1]^=v0[0];
    v1 ^= __builtin_shuffle(v0,Z, (v2di){2,0});//SLL128U(v0,64);
    x->q[0] = v0;
    x->q[1] = v1;
}
static inline
...
//    x->v = __builtin_shuffle(x->v, (v4di){2,3,0,1});
    v2di z  = x->q[1];
    x->q[1] = x->q[0] ^ __builtin_shuffle(x->q[0], z, (v2di){1,2});
    x->q[0] = z;// ^  __builtin_shuffle(z,x->q[1], (v2di){1,2});//(v2di){z[1],x->q[0][0]};
}
